...
3. Соберите и запустите сервисы. Или пересоберите: с флагом ``--build``.
   ...
   docker compose up -d
   ```
4. Когда все сервисы запущены, подготовьте бэкенд к работе. Важно! Если вы изменили файлы в Laravel-проекте (например, сменили версию фреймворка), оставьте ``.env.example`` в изначальном виде!
   ```bash
