...
3. Соберите и запустите сервисы. Или пересоберите: с флагом ``--build``.
   ...
   docker compose up -d
   ```
4. Когда все сервисы запущены, подготовьте бэкенд к работе: создайте файл окружения, установите зависимости, сгенерируйте ключ доступа, выполните миграции, если они есть.
   ```bash
