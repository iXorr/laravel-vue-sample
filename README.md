...
## Начало работы
1. Загрузите зависимости для фронтенда.
   ```
   docker compose build frontend
   docker compose run --rm frontend npm install
   ```
2. Соберите и запустите сервисы. Или пересоберите: с флагом ``--build``.
   ```bash
   docker compose up -d
   ```
3. Когда все сервисы запущены, подготовьте бэкенд к работе. 
